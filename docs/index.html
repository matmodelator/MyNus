...
splitBtn.addEventListener(
  ...
  async () => {
    ...
    // The selected file was already cleared through clearProjectState().
    // Play List state is intentionally preserved.
...
studio.style.display =
  "block";
      ...
      // Separation is complete. Offer Project creation for full Karaoke work.
...
// ========================================
// PROJECT / FULL SCREEN PLAYLIST | 5.2.2
...
function setProjectActive(projectId = null) {
  ...
  if (editLyricsBtn) editLyricsBtn.disabled = !enabled;
  if (openLyricsFullScreenBtn) openLyricsFullScreenBtn.disabled = !enabled;
}
...
async function saveCurrentProject(projectName, savePath, conflictAction = "", options = {}) {
  ...
  if (options.activateProject) {
    ...
    await markProjectInUse(data.id || name, "projects");
  }
...
async function markProjectInUse(id, source = "projects", mode = null) {
  const response = await fetch("/projects/use", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ id, source, mode })
  });
  const data = await response.json().catch(() => ({}));
...
function renderFullScreenPlaylist(projects = karaokePlaylistSnapshot) {
  ...
  const appendTrackRow = (track, source) => {
    ...
    row.addEventListener("click", async event => {
      ...
      // Emergency manual takeover through Play List:
      // loaded Track becomes 0; previous 0 becomes +1.
      await loadProjectById(track.id, true, source, "takeover");
...
function clearProjectState() {
  // Clear ALL current working Project/file state.
  // Play List is deliberately excluded: playlist.json/current/queue/history
  // and the in-memory Play List snapshot are owned by Play List logic only.
  pauseAll();
  karaokeFinishTriggered = false;
  closeKaraokeFinishedDialog();
...
async function loadProjectById(id, keepFullScreen = false, source = "projects", playlistUseMode = null) {
  ...
  try {
    ...
    // Actual Project data is loaded at this point. Close the blocking loader
    // immediately; FX and Play List bookkeeping continue without blocking UI.
    ...
    if (source === "playlist" || source === "history") {
      await markProjectInUse(data.id || id, source, playlistUseMode);
    ...
    if (lyricsFullScreenMode || keepFullScreen) {
...
confirmSaveProjectBtn?.addEventListener("click", async () => {
  ...
  try {
    await saveCurrentProject(name, savePath, "", { activateProject: projectCreationPending });
    projectCreationPending = false;
    saveProjectDialog.classList.remove("open");
  } catch (error) {
    console.error(error);
...
function openNewProjectSaveDialog() {
  if (!currentOriginalFile) {
    status.textContent = "Open and separate an audio file first";
    return;
  }
...
loadProjectBtn?.addEventListener("click", openNewProjectSaveDialog);

createNewProjectBtn?.addEventListener("click", () => {
  newProjectPromptDialog?.classList.remove("open");
  openNewProjectSaveDialog();
});
...
overwriteProjectBtn?.addEventListener("click", async () => {
  ...
  try {
    await saveCurrentProject(pending.name, pending.savePath, "overwrite", { activateProject: projectCreationPending });
    projectCreationPending = false;
...
saveProjectCopyBtn?.addEventListener("click", async () => {
  ...
  try {
    await saveCurrentProject(pending.name, pending.savePath, "copy", { activateProject: projectCreationPending });
    projectCreationPending = false;
    projectConflictDialog.classList.remove("open");
    saveProjectDialog.classList.remove("open");
    pendingProjectConflict = null;
...
lyricsFullScreenPlaylistBtn?.addEventListener("click", event => {
  ...
  // Open immediately from the last known state; refresh from server in background.
  renderFullScreenPlaylist(karaokePlaylistSnapshot);
  refreshFullScreenPlaylist().catch(error => {
    console.error(error);
    status.textContent = error.message;
...
clearProjectState() вызывается при выборе любого нового обычного файла и при любой загрузке Project. | 5.2.2
</div>
