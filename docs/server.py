<!-- MyNus 5.3.0 | Три независимых сценария выбора Project -> один прямой вызов единой функции LOAD; русские комментарии синхронизированы с серверным TRACE  ...ЗАГРУЗКА СТРУКТУРЫ ОДИНАКОВА ДЛЯ ВЕХ СПОСОБОВ! --> 
<!DOCTYPE html>
<html lang="en">
...
async function markProjectInUse(id, source = "projects", mode = null) {
  const response = await fetch("/projects/use", {
    ...
    body: JSON.stringify({ id, source, mode, trace_id: projectLoadTraceId || null })
  });
  const data = await response.json().catch(() => ({}));
...
function renderFullScreenPlaylist(projects = karaokePlaylistSnapshot) {
  ...
  const appendTrackRow = (track, source) => {
    ...
    row.addEventListener("click", async event => {
      ...
      // =====================================================================
      beginProjectLoadTrace("[5.3.0][СЦЕНАРИЙ 2 — PLAYLIST] Выбран Project; дальше прямой переход в единую LOAD-функцию", track.id, "playlist");
      await reportProjectLoadTrace("[5.3.0][СЦЕНАРИЙ 2 — PLAYLIST] Выбран Project; дальше прямой переход в единую LOAD-функцию", {
      ...
      // После успешной единой загрузки PlayList отдельно перестраивает только current/queue/history.
      // Это НЕ часть LOAD и не передаёт никаких параметров в loadProjectById().
      await markProjectInUse(track.id, source, "takeover");
      await refreshFullScreenPlaylist();
...
async function runKaraokeFinishAction(action) {
  ...
  try {
    ...
    if (action === "again") {
      ...
      // =====================================================================
      beginProjectLoadTrace("[5.3.0][СЦЕНАРИЙ 3 — OPEN NEXT] Выбран следующий Project; дальше прямой переход в единую LOAD-функцию", nextProject.id, "playlist");
      await reportProjectLoadTrace("[5.3.0][СЦЕНАРИЙ 3 — OPEN NEXT] Выбран следующий Project; дальше прямой переход в единую LOAD-функцию", {
      ...
      if (loaded) {
        // После успешной единой загрузки PlayList отдельно двигает current/queue/history.
        // Это НЕ часть LOAD и не меняет работу loadProjectById().
        await markProjectInUse(nextProject.id, "playlist");
        await refreshFullScreenPlaylist();
        karaokeFinishTriggered = false;
...
function restoreProjectLyricsJson(lyrics) {
  ...
  lyricsStructureDraft =
  Array.isArray(lyrics?.structure) && lyrics.structure.length > 0
...
// Диагностика полной цепочки LOAD Project -> Full Screen | MyNus 5.3.0.
// Каждая запись видна в HTML Service Console и отправляется серверу для CMD trace.
const MYNUS_CLIENT_VERSION = "5.3.0";
const MYNUS_CLIENT_CHANGE = "Три сценария выбора Project -> одна функция LOAD; русские комментарии синхронизированы с серверным TRACE";
...
// Сохранение текущего Project и связанных данных.
async function showProjectLoader() {
  projectLoading = true;
...
async function loadProjectById(id) {
  if (!projectLoadTraceId || projectLoadTraceProject !== (id || null)) {
    beginProjectLoadTrace("[5.3.0][ЕДИНАЯ LOAD-ФУНКЦИЯ] Начало единой загрузки Project по ID", id || null, "projects");
  } else {
    projectLoadTraceProject = id || projectLoadTraceProject;
  }

  await reportProjectLoadTrace("[5.3.0][ЕДИНАЯ LOAD-ФУНКЦИЯ] Начало единой загрузки Project по ID", {
  ...
  try {
    ...
    // uiMode is deliberately NOT changed by LOAD.
    ...
    // Restore Structure and its timeline as one visual unit.
    ...
    // uiMode remains exactly what it was before LOAD.
    // Play List bookkeeping is intentionally outside Project LOAD.
    return true;
  } catch (error) {
  ...
  if (!projects.length) {
    ...
    projects.forEach(project => {
      ...
      button.addEventListener("click", async () => {
        try {
          // =====================================================================
          // [5.3.0][СЦЕНАРИЙ 1 — OPEN PROJECT]
          // Пользователь выбрал Project в обычном окне Open Project.
          // Этот сценарий выполняет только выбор Project и получает его ID.
          // Никаких специальных параметров загрузки, отдельных LOAD-функций или веток нет.
          // После определения ID выполняется прямой переход в ЕДИНУЮ функцию loadProjectById(ID).
          // =====================================================================
          beginProjectLoadTrace("[5.3.0][СЦЕНАРИЙ 1 — OPEN PROJECT] Выбран Project; дальше прямой переход в единую LOAD-функцию", project.id, "projects");
          await reportProjectLoadTrace("[5.3.0][СЦЕНАРИЙ 1 — OPEN PROJECT] Выбран Project; дальше прямой переход в единую LOAD-функцию", {
          ...
          // После успешной единой загрузки обычный Open Project отдельно начинает новый PlayList-контекст.
          // Это НЕ часть LOAD и не передаёт никаких параметров в loadProjectById().
          await reportProjectLoadTrace("PLAYLIST CURRENT UPDATE START", { mode: "open" });
          await markProjectInUse(project.id, "projects", "open");
...
openProjectStartBtn?.addEventListener("click", () => {
  beginProjectLoadTrace("Open Project button", null, "projects");
  void reportProjectLoadTrace("OPEN PROJECT BUTTON CLICK");
