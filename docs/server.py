# ========================================
# MyNus Server 5.3.0 | Русский TRACE трёх сценариев выбора Project и единой LOAD-функции
# ========================================
...
# [5.3.0][СЦЕНАРИЙ 1 — OPEN PROJECT] Выбран Project; дальше прямой переход в единую LOAD-функцию
# [5.3.0][СЦЕНАРИЙ 2 — PLAYLIST] Выбран Project; дальше прямой переход в единую LOAD-функцию
# [5.3.0][СЦЕНАРИЙ 3 — OPEN NEXT] Выбран следующий Project; дальше прямой переход в единую LOAD-функцию
# [5.3.0][ЕДИНАЯ LOAD-ФУНКЦИЯ] Начало единой загрузки Project по ID
#
...
def debug_load_trace():
    ...
    return jsonify({"ok": True, "version": "5.3.0", "trace_id": trace_id})
...
# ========================================
# MYNUS PlayList JSON state + standalone Projects | 5.3.0
# ========================================
...
# =====================================================================
# [5.3.0][PLAYLIST — ТОЛЬКО ОЧЕРЕДЬ]
# Этот маршрут НЕ загружает Project.
# Он вызывается отдельно ПОСЛЕ успешной единой LOAD-функции и меняет только
# current / queue / history в playlist.json.
# Параметры source/mode существуют ТОЛЬКО для правил перестановки очереди
# и никогда не передаются в loadProjectById().
# =====================================================================
def use_project():
    data = request.get_json(silent=True) or {}
...
# =====================================================================
# [5.3.0][ЕДИНАЯ LOAD-ФУНКЦИЯ] Сервер отдаёт Project по одному ID.
# Этот маршрут одинаков для всех трёх сценариев index.
# Сервер НЕ получает source/mode и НЕ знает, откуда пришёл ID:
# Open Project, PlayList или Open next.
# =====================================================================
def get_saved_project(project_id):
    project_id = _project_id(project_id)
    print(
        f"[5.3.0][ЕДИНАЯ LOAD-ФУНКЦИЯ] Сервер получил GET Project по ID: {project_id}",
        flush=True
    )
    folder = os.path.join(PROJECTS_DIR, project_id)
    project_path = os.path.join(folder, "Project.json")
...
if __name__ == "__main__":
    print("\n" + "=" * 72)
    print("MyNus Server 5.3.0")
    print(r"5.3.0: три сценария выбора Project; единая LOAD-функция получает только ID; русские маркеры index выводятся сервером в CMD.")
    print("=" * 72 + "\n")
