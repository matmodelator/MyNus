...
/* ========================================
    ...
    * {
    ...
    .lyrics-editor-actions { display: flex; gap: 6px; }
    ...
    .karaoke-project-loader.open { display: flex; }
    .karaoke-project-loader-card {
    ...
    .karaoke-project-loader-title {
      font-size: 24px;
      font-weight: 800;
      text-align: center;
      margin-bottom: 18px;
    }
    .karaoke-project-loader-phase {
      min-height: 24px;
      font-size: 16px;
      text-align: center;
      color: #ddd;
    ...
    .karaoke-project-loader-percent {
      margin-top: 10px;
    ...
    .karaoke-project-loader.failed .karaoke-project-loader-phase { color: #ff8a8a; }
    ...
    .lyrics-fullscreen-side-control.left { left: 24px; }
    .lyrics-fullscreen-side-control.right { right: 24px; }
    .lyrics-fullscreen-side-icon { width: 38px; height: 32px; padding: 0; font-size: 18px; }
    ...
    .lyrics-fullscreen-vertical-scale .scale-value { font: 700 12px Consolas, monospace; color:#ddd; }
    ...
    /* Separate upper row for the real start/end values. It occupies its own
       height and does not change Structure/timeline width or time mapping. */
    ...
    /* Endpoint tick anchors stay exactly at 0% / 100%; only their labels
       live in the separate upper row. */
    ...
    /* LYRICS EDITOR — KARAOKE WORDS | 4.4.0 */
    ...
    <div class="transport-stack transport-top">
      ...
      <div class="transport-band">
        <div class="shortcut-legend">
          <button type="button" id="loadProjectBtn" class="project-side-btn">Load Project</button>
...
<!-- ========================================
    ...
    <div class="track generated-track" id="pitchCorrectionTrackRow">
      <div class="track-controls">
        ...
        <div class="track-save-row"><button data-export-track="pitchCorrection" data-export-scope="full">Save As...</button><button data-export-track="pitchCorrection" data-export-scope="selection">Save Selection</button></div>
    ...
    <div class="track generated-track" id="harmonizerTrackRow">
      <div class="track-controls">
        ...
        <div class="track-save-row"><button data-export-track="harmonizer" data-export-scope="full">Save As...</button><button data-export-track="harmonizer" data-export-scope="selection">Save Selection</button></div>
    ...
    <div class="track">
      ...
      <div class="track-controls master-controls">
        ...
        <div class="track-buttons">
          <button id="saveAsMasterBtn" disabled>Save As...</button>
          <button id="saveMasterSelectionBtn" disabled>Save Selection</button>
        </div>

        <div class="track-buttons">
          <button id="undoMasterBtn" disabled>Undo</button>
          <button id="redoMasterBtn" disabled>Redo</button>
        </div>
    ...
    <div class="track">
      ...
      <div class="lyrics-controls">
        <button type="button" id="editLyricsBtn">Lyrics Editor</button>
        <button type="button" id="openLyricsFullScreenBtn">Full Screen</button>
      </div>
...
<div id="lyricsEditorDialog" class="lyrics-editor-dialog">
  <div class="lyrics-editor-card">
    ...
    <div id="lyricsFullScreenView" class="lyrics-fullscreen-view" aria-hidden="true">
      <div id="karaokeProjectLoader" class="karaoke-project-loader" aria-live="polite" aria-hidden="true">
        <div class="karaoke-project-loader-card">
          <div class="karaoke-project-loader-title">Loading project</div>
          <div id="karaokeProjectLoaderPhase" class="karaoke-project-loader-phase">Opening project</div>
          <div class="karaoke-project-loader-track"><div id="karaokeProjectLoaderFill" class="karaoke-project-loader-fill"></div></div>
          <div id="karaokeProjectLoaderPercent" class="karaoke-project-loader-percent">0%</div>
        </div>
      </div>
    ...
    <div class="lyrics-editor-toolbar">
      ...
      <div class="lyrics-editor-toolbar-center">
        <span id="lyricsLanguageInfo">Language: RU</span>
        <button type="button" id="lyricsTranscriptionBtn" disabled>
          RU Transcription
        </button>
      </div>
    ...
    <div class="lyrics-editor-footer">
      ...
      <div class="lyrics-editor-actions">
        ...
        <button type="button" id="saveLyricsEditBtn">Save</button>
        <button type="button" id="lyricsSaveAsBtn">Save As...</button>
      </div>
...
window.addEventListener("error", event => {
  if (document.getElementById("lyricsEditorDialog")?.classList.contains("open") ||
...
// Shared text marker in Lines: boundary before word N.
...
function drawMasterTimeline() {
  ...
  // Keep adjacent visible divisions at least about 1 cm apart (~38 CSS px).
...
function playCurrentMode() {
  ...
  const playbackEnd =
    mode === "master" && masterDuration > 0
      ? masterDuration
      : duration;

  if (lyricsFullScreenMode && playbackEnd > 0 && time < playbackEnd - 0.25) {
...
function applyMix() {
  ...
  const anySolo =
    stemIds.some(stemId =>
      lyricsFullScreenMode && lyricsFullScreenTrackState?.[stemId]
  ...
  stemIds.forEach(
    stemId => {
      ...
      const fsState = lyricsFullScreenMode ? lyricsFullScreenTrackState?.[stemId] : null;
  ...
  if (lyricsFullScreenMode) applyLyricsFullScreenTrackFxNodes();
  updatePlayheads();
...
function ensureMixAudioGraph() {
  ...
  masterRecordProcessor.onaudioprocess =
    event => {
      ...
      // Live waveform is derived from the exact PCM being recorded.
      let peak =
        0;
...
async function startMasterRecording() {
  ...
  // Freeze the exact start position on the shared song timeline.
  ...
  // Start playback first, but PCM capture is still off.
  if (
    !isPlaying
  ) {
  ...
  // Re-lock all six stems immediately before capture.
  const lockedStart =
    getCurrentTime();
...
.forEach(
  button => {
    ...
    // Prevent mouse clicks from leaving keyboard focus on Undo/Redo.
    button.addEventListener(
      "mousedown",
      event => {
    ...
    // Space must never activate Undo/Redo.
    button.addEventListener(
      "keydown",
      event => {
...
async function getTrackBlob(track) {
  if (track === "original") {
    return {
      ...
      filename: currentOriginalFile?.name || "original.bin"
    };
  }

  if (stemIds.includes(track)) {
    // Generated effect tracks live in generatedTrackURLs. Use them explicitly.
...
// ========================================
// GENERATED VOCAL EFFECT TRACKS | 4.6.0
// ========================================
...
function updateSelectionDisplay() {
  ...
  selectionRange.style.left =
    startX + "px";


  selectionRange.style.width =
    Math.max(
      1,
      endX - startX
    )
...
// A Lines marker is a TEXT BOUNDARY, not the nearest word centre.
// Boundary N means: words [0..N-1] stay left, words [N..] go right.
...
function leaveLyricsEdit() {
  if (lyricsEditorEditLineIndex < 0) return;
  ...
  if (line && textArea) {
    line._editedText = textArea.value;
...
function updateLyricsFullScreenView(currentTime) {
  if (!lyricsFullScreenMode || !lyricsEditorCard?.classList.contains("karaoke-fullscreen-mode")) return;

  if (lyricsFullScreenTrackName) {
    lyricsFullScreenTrackName.textContent = currentOriginalFile?.name || fileName?.textContent || "No file selected";
  }

  if (lyricsFullScreenCounter) {
    lyricsFullScreenCounter.textContent = "-" + formatTime(vocalRemaining);
    updateLyricsFullScreenCounterStyle(vocalRemaining, vocalPhase);
  }
...
function applyLyricsFullScreenTrackFxNodes() {
  if (!lyricsFullScreenMode || !mixAudioContext) return;
  stemIds.forEach(stemId => {
...
lyricsFullScreenView?.addEventListener("click", event => {
  if (!lyricsFullScreenMode) return;
...
lyricsFullScreenStopBtn?.addEventListener("click", event => {
  ...
  // Full Screen STOP means: finish the song now.
  // Sequencer Stop is intentionally not used here because it returns to 0.
  pauseAll();
...
function setLyricsFullScreenMode(enabled) {
  closeLyricsFullScreenMenus();
  lyricsFullScreenMode = Boolean(enabled);
  lyricsEditorCard?.classList.toggle("karaoke-fullscreen-mode", lyricsFullScreenMode);
  lyricsFullScreenView?.setAttribute("aria-hidden", lyricsFullScreenMode ? "false" : "true");
  ...
  if (lyricsFullScreenMode) {
    ...
    updateLyricsMasterCounter();
    updateLyricsFullScreenView(getCurrentTime());
  } else {
...
lyricsModeSwitchBtn?.addEventListener("click", () => setLyricsFullScreenMode(true));
lyricsFullScreenBackBtn?.addEventListener("click", () => setLyricsFullScreenMode(false));

function updateLyricsEditorKaraoke(currentTime) {
  if (!lyricsEditorDialog.classList.contains("open")) return;
...
function detectLyricsStructure(lines) {
  ...
  // Chorus: repeated contiguous lyric chunks. Prefer longer repeated chunks first.
  ...
  // Verse base T: duration shared by at least two non-chorus vocal chunks within ±1 second.
  let verseBase = null;
  let bestMatches = 0;
  ...
  const lastLyricEnd = cleanLines[cleanLines.length - 1].end;
  if (songEnd - lastLyricEnd >= MIN_INSTRUMENTAL_GAP) {
    classified.push({ type: "Coda", start: lastLyricEnd, end: songEnd });
  }
...
function normalizeStructureCoverage(structure, requestedSongEnd = null) {
  ...
  // Structure is a continuous partition of the song: no invisible gaps.
  items[0].start = 0;
  ...
  // Merging short blocks may change neighbours; restore exact continuity.
  if (items.length) {
    items[0].start = 0;
    for (let i = 1; i < items.length; i++) {
...
function ensureLyricsStructureDraft() {
  if (lyricsStructureDraft !== null) return;
  const detectedStructure = detectLyricsStructure(lyricsEditorDraft);
  lyricsStructureDraft = normalizeStructureCoverage(
    detectedStructure,
    structureSongEnd(detectedStructure)
  ).map(item => ({ ...item }));
...
function openLyricsStructureNameMenu(event, index) {
  ...
  // Menu height follows its contents: no internal scroll.
...
function playLyricsStructureTimelineAt(event) {
  ...
  // One shared cursor for Structure, Lines and Sequencer. Transport state is preserved.
  setGlobalCursorTime(time);
...
function renderLyricsStructureTimeline(songEnd) {
  ...
  if (lyricsStructureStartValue) {
    lyricsStructureStartValue.textContent = formatStructureTime(0);
  }
  if (lyricsStructureEndValue) {
    lyricsStructureEndValue.textContent = formatStructureTime(total);
  }
  let step = 15;
  ...
  times.forEach((t, index) => {
    ...
    if (isStart || isEnd) {
      ...
      // Numeric endpoint values are rendered in the separate upper row.
      // This span remains only as the exact 0% / 100% scale anchor.
    } else {
...
function renderLyricsStructure() {
  ...
  structure.forEach((segment, index) => {
    ...
    if (lyricsStructureSelectedIndices.has(index) || index === lyricsStructureSelectedIndex) {
      block.classList.add("selected");
    }

    const structureName = String(segment.label || segment.type || "").trim();
    block.textContent = structureName || "[      ]";
    block.title =
      (structureName || "[      ]")
      + "  "
    ...
    block.addEventListener("click", event => {
      ...
      // Structure and Lines never compete for Split: only one selection is active.
      lyricsEditorSelectedLine = -1;
      document.querySelectorAll(".lyrics-editor-row").forEach(item => item.classList.remove("selected"));
      setLyricsStructureSelection([index]);

      // If Structure Loop is already active, selecting another block retargets it.
...
function renderLyricsEditor() {
  ...
  lyricsEditorDraft.forEach((line, index) => {
    ...
    startInput.addEventListener("change", event => {
      const value = parseLyricsTime(event.target.value);
      if (Number.isFinite(value)) line.start = value;
      event.target.value = formatLyricsTime(line.start);
    });

    endInput.addEventListener("change", event => {
      const value = parseLyricsTime(event.target.value);
      if (Number.isFinite(value)) line.end = value;
      event.target.value = formatLyricsTime(line.end);
    });
    ...
    karaokeBox.addEventListener("click", event => {
      ...
      if (wordSpan) {
        // POINTER: a word click is transport only.
        if (lyricsEditorEditLineIndex >= 0) leaveLyricsEdit();
        seekLyricsWord(line, Number(wordSpan.dataset.wordIndex));
      } else {
        // CARET: a click between words enters text editing only; transport time is untouched.
...
function updateLyricsTransportButtons() {
  if (!lyricsEditorDialog || !lyricsEditorDialog.classList.contains("open")) return;
...
function updateLyricsFullScreenCounterStyle(remaining, phase) {
  ...
  /*
  Full Screen does NOT calculate V.
  It only displays the same vocalRemaining already calculated by Sequencer.
  The phase is used only to preserve the existing Full Screen styling.
  */
  if (phase !== "pre") {
  ...
  if (value <= 0) {
    lyricsFullScreenCounter.classList.remove("pre-vocal");
    lyricsFullScreenCounter.classList.add("post-vocal");
    return;
  }
...
function updateLyricsMasterCounter() {
  if (!lyricsMasterCounter) return;

  const sequencerCounter =
    String(timeDisplay?.textContent || timeDisplayTop?.textContent || "");
  const vocalMatch =
    sequencerCounter.match(/(?:^|\s\/\s)V:([^\s/]+)/);

  lyricsMasterCounter.textContent =
    vocalMatch ? vocalMatch[1] : "-00:00.00";
...
function openLyricsEditor() {
  lyricsFullScreenMode = false;
  lyricsEditorCard?.classList.remove("karaoke-fullscreen-mode");
  lyricsFullScreenView?.setAttribute("aria-hidden", "true");
  lyricsStructureDraft = null;
  lyricsStructureSelectedIndex = -1;
  lyricsTranscriptionMode = false;
  ...
  lyricsEditorDialog.classList.add("open");
}

function closeLyricsEditor() {

  setLyricsFullScreenMode(false);
  ...
  lyricsEditorDialog.classList.remove(
    "open"
  );
...
lyricsPlayLineBtn.addEventListener(
  "click",
  () => {
    // Structure selected -> Loop works on the selected structure block.
...
lyricsSaveAsBtn.addEventListener("click", () => {
  const defaultName =
    currentOriginalFile
    ? currentOriginalFile.name.replace(
        /\.[^.]+$/,
        ""
      )
    : "lyrics";
...
function exportLyricsFile(rawName, format, encoding = "utf8") {
  ...
  if (format === "json") {
    ...
    const structureBody = structureScheme
      ? "<div class='structure'>"
        + structureScheme
            .split("\n")
            .map(row => "<div>" + escapeLyricsHtml(row) + "</div>")
            .join("")
        + "</div>"
      : "";

    // Print through a temporary hidden iframe instead of a popup window.
    // The iframe is removed after printing, so the editor cannot remain stuck
    // behind a dead/foreground print window after PDF save/cancel.
    ...
    let printCleaned = false;
    const cleanupPrintFrame = () => {
      if (printCleaned) return;
      printCleaned = true;
    ...
    printDocument.write(
      ...
      + "<title>" + escapeLyricsHtml(safeName) + "</title>"
      + "<style>"
      + "body{font-family:Arial,sans-serif;padding:40px;line-height:1.5;color:#111}"
      + "h1{font-size:22px;margin:0 0 28px}"
      + "p{margin:0 0 8px}"
      + ".structure{margin-top:28px;padding-top:16px;border-top:1px solid #999;font-family:Consolas,monospace;font-size:12px;white-space:pre}"
      + ".structure div{margin:0 0 4px}"
      + "</style></head><body>"
      + "<h1>" + escapeLyricsHtml(safeName) + "</h1>"
      + body
      + structureBody
      + "</body></html>"
    );
...
saveLyricsEditBtn.addEventListener(
  "click",
  async () => {
    try {
      const lyricsJson = projectLyricsJson();
      activeKaraokeLine = -1;
      renderLyrics();
      renderLyricsEditor();

      showLyricsSaveNotice("Saving…");
      setLyricsEditorMessage("LYRICS: saving Lyrics.json…");

      const response = await fetch("/lyrics/save-current", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ lyrics: lyricsJson })
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Lyrics save failed");

      showLyricsSaveNotice("Lyrics saved successfully");
      setLyricsEditorMessage("LYRICS: saved Lyrics.json — " + (data.path || "current project"));
    } catch (error) {
      console.error(error);
      showLyricsSaveNotice("Lyrics save failed", true);
      setLyricsEditorError(error, "LYRICS SAVE: ");
    }
  }
);
...
function startAnimation() {
  ...
  const update =
    () => {
      ...
      // Audio transport is the source while playing; all three visual cursors use this same value.
...
function changeZoom(
  ...
  // Audio position is the source of truth.
  // Zoom never seeks; only redraw coordinates from the same time.
  updatePlayheads();
...
// Dedicated Tab switch: Lyrics Editor <-> Full Screen Karaoke.
// Capture phase prevents browser focus traversal and focused controls from
// consuming Tab before MyNus handles it.
window.addEventListener(
  ...
  event => {
    ...
    const lyricsOpen = Boolean(lyricsEditorDialog?.classList.contains("open"));
    const karaokeVisible = Boolean(lyricsFullScreenMode)
      || lyricsFullScreenView?.getAttribute("aria-hidden") === "false"
      || Boolean(lyricsEditorCard?.classList.contains("karaoke-fullscreen-mode"));

    if (!lyricsOpen && !karaokeVisible) return;

    event.preventDefault();
    event.stopImmediatePropagation();
    setLyricsFullScreenMode(!karaokeVisible);
  },
  true
);
...
window.addEventListener(
  ...
  event => {
    ...
    // Song finished: Space = Play again.
    ...
    // Lyrics Editor / Full Screen: Esc always exits directly to Sequencer.
    if (event.code === "Escape" && lyricsEditorDialog?.classList.contains("open")) {
      event.preventDefault();
      closeLyricsEditor();
      return;
    }

    // Full Screen-only navigation and popup controls.

    if (lyricsFullScreenMode && event.code === "Backspace") {
      event.preventDefault();
      closeLyricsFullScreenMenus();
      return;
    }

    if (lyricsFullScreenMode && event.code === "PageUp") {
      event.preventDefault();
      setLyricsFullScreenMeters(true);
      return;
    }

    if (lyricsFullScreenMode && event.code === "PageDown") {
      event.preventDefault();
      setLyricsFullScreenMeters(false);
      return;
    }

    // Full Screen: Space always controls transport, regardless of Lines EDIT/caret.
    if (event.code === "Space") {
      if (lyricsFullScreenMode) {
        event.preventDefault();
      ...
      // Lyrics Editor: active EDIT/caret keeps Space as text input.
...
// ========================================
// PROJECT / FULL SCREEN PLAYLIST | 5.1.18
// Project = 12 audio slots + Lyrics.json + Project.json
// ========================================
...
// Snapshot the CURRENT Lyrics state for Project SAVE.
// Unsaved text currently typed in Lyrics Editor is committed first.
function projectLyricsJson() {
  ...
  // Keep the live Lyrics state synchronized with the snapshot being saved.
...
async function saveCurrentProject(projectName, savePath, conflictAction = "", options = {}) {
  ...
  // Use the existing per-track save source: getTrackBlob().
  // Nine current audio tracks are saved; three reserve slots stay reserved.
  const workingTrackIds = projectTrackIds.slice(0, 9);
...
async function markProjectInUse(id, source = "projects") {
...
async function refreshFullScreenPlaylist() {
  ...
  const historyProjects = projects.filter(project => project.status === "executed");
  const waitingProjects = projects.filter(project => project.status === "waiting");

  const appendProjectRow = (project, source) => {
    const row = document.createElement("button");
    row.type = "button";
    row.className = "lyrics-fullscreen-playlist-item " + (project.status || "waiting");
    const position = Number(project.position);
    const numberLabel = Number.isFinite(position)
      ? (position > 0 ? "+" + position : String(position))
      : "";
    row.textContent = (numberLabel ? numberLabel + "  " : "") + project.name;
    row.title = project.name + " — load project; right-click — remove from PlayList";

    if (source === "playlist" && Number.isFinite(position)) {
      row.draggable = true;
      ...
      row.addEventListener("drop", async event => {
        ...
        const to = Number(row.dataset.queuePosition);
        if (!Number.isFinite(from) || !Number.isFinite(to) || from === to) return;
        await movePlaylistProject(from, to);
    ...
    row.addEventListener("contextmenu", async event => {
      ...
      if (!window.confirm("Remove " + project.name + " from PlayList?")) return;
    ...
    row.addEventListener("click", async event => {
      event.stopPropagation();
      await loadProjectById(project.id, true, source);
      lyricsFullScreenPlaylistMenu.hidden = true;
    });
    lyricsFullScreenPlaylistMenu.appendChild(row);
  };

  historyProjects.forEach(project => appendProjectRow(project, "history"));
  ...
  waitingProjects.forEach(project => appendProjectRow(project, "playlist"));
  ...
  addButton.className = "lyrics-fullscreen-playlist-item waiting";
  addButton.textContent = "+ Add Project";
  addButton.title = "Add project from Projects to queue";
  addButton.addEventListener("click", async event => {
    event.stopPropagation();
    const saved = await fetchSavedProjects();
    if (!saved.length) {
      status.textContent = "Projects is empty";
      return;
    }
    const list = saved.map((project, index) => (index + 1) + ". " + project.name).join("\n");
    const choice = window.prompt("Add Project to PlayList:\n" + list, "1");
    if (choice === null) return;
    const selectedIndex = Number(choice);
    const selected = Number.isInteger(selectedIndex)
      ? saved[selectedIndex - 1]
      : saved.find(project => project.name === choice || project.id === choice);
    if (!selected) throw new Error("Project not found");

    const defaultPosition = waitingProjects.length + 1;
    const positionValue = window.prompt(
      "Queue position for " + selected.name + " (+1 ... +" + defaultPosition + "):",
      String(defaultPosition)
    );
    if (positionValue === null) return;
    const position = Number(positionValue);
    if (!Number.isInteger(position) || position < 1 || position > defaultPosition) {
      throw new Error("Invalid queue position");
    }
    await addProjectToPlaylist(selected.id, position);
    await refreshFullScreenPlaylist();
  });
  lyricsFullScreenPlaylistMenu.appendChild(addButton);

  if (!historyProjects.length && !currentPlaylistProject && !waitingProjects.length) {
...
function karaokeHasNextProject() {
  return karaokePlaylistSnapshot.some(project => project.status === "waiting");
}
...
function openKaraokeFinishedDialog() {
  if (!lyricsFullScreenMode || !karaokeFinishedDialog || karaokeFinishedDialog.classList.contains("open")) return;
...
async function runKaraokeFinishAction(action) {
  ...
  try {
    ...
    if (action === "again") {
      ...
      const projects = await fetchPlaylistProjects();
      const nextProject = projects.find(project => project.status === "waiting") || null;
      if (!nextProject) throw new Error("No next project in playlist");
      closeKaraokeFinishedDialog();
      const loaded = await loadProjectById(nextProject.id, true, "playlist");
      if (loaded) {
        karaokeFinishTriggered = false;
      } else if (lyricsFullScreenMode && karaokeFinishedDialog) {
        karaokeFinishedDialog.classList.add("open");
      }
    } else if (action === "other") {
      closeKaraokeFinishedDialog();
      const loaded = await loadProjectById("", true, "folder");
      if (loaded) {
        karaokeFinishTriggered = false;
      } else if (lyricsFullScreenMode && karaokeFinishedDialog) {
        karaokeFinishedDialog.classList.add("open");
    ...
    console.error(error);
    if (lyricsFullScreenMode && karaokeFinishTriggered && karaokeFinishedDialog) {
      karaokeFinishedDialog.classList.add("open");
    }
...
karaokeFinishedCancelBtn?.addEventListener("click", closeKaraokeFinishedDialog);
...
function restoreProjectLyricsJson(lyrics) {
  ...
  lyricsStructureDraft = Array.isArray(lyrics?.structure) ? cloneSerializable(lyrics.structure, []) : [];
  lyricsData = {
...
const karaokeProjectLoader = document.getElementById("karaokeProjectLoader");
const karaokeProjectLoaderPhase = document.getElementById("karaokeProjectLoaderPhase");
const karaokeProjectLoaderFill = document.getElementById("karaokeProjectLoaderFill");
const karaokeProjectLoaderPercent = document.getElementById("karaokeProjectLoaderPercent");
let karaokeProjectLoading = false;

const karaokeProjectLoadPhases = [
...
function setKaraokeProjectLoadPhase(index) {
  if (!karaokeProjectLoader) return;
  const phase = karaokeProjectLoadPhases[Math.max(0, Math.min(index, karaokeProjectLoadPhases.length - 1))];
  karaokeProjectLoader.classList.remove("failed");
  karaokeProjectLoaderPhase.textContent = phase.label;
  karaokeProjectLoaderFill.style.width = phase.percent + "%";
  karaokeProjectLoaderPercent.textContent = phase.percent + "%";
}

async function showKaraokeProjectLoader() {
  karaokeProjectLoading = true;
  karaokeProjectLoader?.classList.add("open");
  karaokeProjectLoader?.setAttribute("aria-hidden", "false");
  setKaraokeProjectLoadPhase(0);
  await new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)));
}

async function advanceKaraokeProjectLoad(index, holdMs = 0) {
  if (!karaokeProjectLoading) return;
  setKaraokeProjectLoadPhase(index);
  await new Promise(resolve => requestAnimationFrame(resolve));
  if (holdMs > 0) await new Promise(resolve => setTimeout(resolve, holdMs));
}

async function finishKaraokeProjectLoader() {
  if (!karaokeProjectLoading) return;
  await advanceKaraokeProjectLoad(10, 320);
  karaokeProjectLoader?.classList.remove("open");
  karaokeProjectLoader?.setAttribute("aria-hidden", "true");
  karaokeProjectLoading = false;
}

async function failKaraokeProjectLoader(error) {
  if (!karaokeProjectLoading || !karaokeProjectLoader) return;
  karaokeProjectLoader.classList.add("failed");
  karaokeProjectLoaderPhase.textContent = "Project load failed";
  karaokeProjectLoaderPercent.textContent = "";
  await new Promise(resolve => setTimeout(resolve, 900));
  karaokeProjectLoader.classList.remove("open", "failed");
  karaokeProjectLoader.setAttribute("aria-hidden", "true");
  karaokeProjectLoading = false;
}

async function loadProjectById(id, keepFullScreen = false, source = "projects") {
  const useKaraokeLoader = !!keepFullScreen;
  pauseAll();
  status.textContent = "Loading project...";
  if (useKaraokeLoader) await showKaraokeProjectLoader();

  try {
    // Every entry point uses the SAME full project loader below.
    // Only the source of Project.json / Lyrics.json / tracks is different.
    let response;
    if (source === "folder") {
      response = await fetch("/projects/open-folder", { method: "POST" });
    } else {
      const projectUrl = source === "playlist"
        ? "/projects/" + encodeURIComponent(id)
        : source === "history"
          ? "/history-projects/" + encodeURIComponent(id)
          : "/saved-projects/" + encodeURIComponent(id);
    ...
    if (useKaraokeLoader) await advanceKaraokeProjectLoad(1);

    // Original is restored as the Original track.
    const originalResponse = await fetch(tracks.original);
    ...
    currentOriginalFile = originalFile;
    recentSelectedAudioFile = originalFile;
    fileName.textContent = data.name || id;
    if (source === "folder") currentPlaylistProject = null;
    ...
    // Decorative phase is deliberately shown even when stems already exist in the saved project.
    if (useKaraokeLoader) await advanceKaraokeProjectLoad(2, 180);
    if (useKaraokeLoader) await advanceKaraokeProjectLoad(3);

    // Restore every current stem directly into its existing MyNus track.
    // Generated effect tracks are ordinary saved tracks on LOAD; they do not need regeneration.
    ...
    if (useKaraokeLoader) await advanceKaraokeProjectLoad(4);
    ...
    if (useKaraokeLoader) await advanceKaraokeProjectLoad(5);
    ...
    if (useKaraokeLoader) await advanceKaraokeProjectLoad(6);
    restoreProjectLyricsJson(lyrics);
    renderLyrics();
    renderLyricsEditor();

    if (useKaraokeLoader) await advanceKaraokeProjectLoad(7);
    renderLyricsStructureTimeline(duration);

    updateCropButtons();
    updateMixButtons();
    studio.style.display = "block";

    if (useKaraokeLoader) await advanceKaraokeProjectLoad(8);
    if (keepFullScreen) {
      if (!lyricsEditorDialog.classList.contains("open")) openLyricsEditor();
      setLyricsFullScreenMode(true);
    }

    if (useKaraokeLoader) await advanceKaraokeProjectLoad(9);
    stemIds.forEach(stemId => applyFxSettings(stemId));
    applyMix();

    if (source === "playlist" || source === "history") {
      await markProjectInUse(data.id || id, source);
    } else if (source === "projects") {
      await markProjectInUse(data.id || id, "projects");
    }
    // A folder opened outside Projects is intentionally not written to playlist.json:
    // playlist.json contains stable references to Projects only.

    status.textContent = "Project loaded: " + (data.name || id);

    if (lyricsFullScreenMode || keepFullScreen) await refreshFullScreenPlaylist();
    if (useKaraokeLoader) await finishKaraokeProjectLoader();
    return true;
  } catch (error) {
    if (useKaraokeLoader) await failKaraokeProjectLoader(error);
    throw error;
  }
}
  ...
  if (!projects.length) {
    ...
    projects.forEach(project => {
      ...
      button.addEventListener("click", async () => {
        try {
          await loadProjectById(project.id, false, "projects");
...
saveProjectBtn?.addEventListener("click", () => {
  ...
  if (saveProjectPathInput && !saveProjectPathInput.value.trim()) saveProjectPathInput.value = "C:\\MyNus\\Projects";
  if (saveProjectMessage) saveProjectMessage.textContent = "";
...
browseSaveProjectPathBtn?.addEventListener("click", async () => {
  ...
  try {
    const response = await fetch("/projects/select-folder", { method: "POST" });
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data.error || "Folder selection failed");
    if (data.path) saveProjectPathInput.value = data.path;
  } catch (error) {
    console.error(error);
    if (saveProjectMessage) saveProjectMessage.textContent = error.message;
  } finally {
...
saveProjectDialog?.addEventListener("click", event => {
  if (event.target === saveProjectDialog) saveProjectDialog.classList.remove("open");
});
...
confirmSaveProjectBtn?.addEventListener("click", async () => {
  ...
  if (saveProjectMessage) saveProjectMessage.textContent = "Saving…";
  try {
    ...
    if (error.projectConflict) {
      pendingProjectConflict = { name, savePath };
      if (projectConflictMessage) projectConflictMessage.textContent = "Project “" + name + "” already exists.";
...
overwriteProjectBtn?.addEventListener("click", async () => {
  if (!pendingProjectConflict) return;
  const pending = pendingProjectConflict;
  overwriteProjectBtn.disabled = true;
  try {
    await saveCurrentProject(pending.name, pending.savePath, "overwrite");
...
saveProjectCopyBtn?.addEventListener("click", async () => {
  ...
  try {
    await saveCurrentProject(pending.name, pending.savePath, "copy");
    projectConflictDialog.classList.remove("open");
    saveProjectDialog.classList.remove("open");
    pendingProjectConflict = null;
  } catch (error) {
...
renameProjectBtn?.addEventListener("click", () => {
  ...
  pendingProjectConflict = null;
  saveProjectDialog.classList.add("open");
  if (saveProjectMessage) saveProjectMessage.textContent = "Enter a new project name.";
  saveProjectNameInput.focus();
  saveProjectNameInput.select();
});
...
loadProjectDialog?.addEventListener("click", event => {
  if (event.target === loadProjectDialog) loadProjectDialog.classList.remove("open");
});

lyricsFullScreenPlaylistBtn?.addEventListener("click", async event => {
  event.stopPropagation();
  try {
    await refreshFullScreenPlaylist();
    lyricsFullScreenPlaylistMenu.hidden = !lyricsFullScreenPlaylistMenu.hidden;
  } catch (error) {
    console.error(error);
    status.textContent = error.message;
  }
...
PlayList Full Screen: состояние хранится только в C:\MyNus\playlist.json: current / queue / history. Проекты физически хранятся только в C:\MyNus\Projects. + Add Project позволяет вставить проект в любую позицию +N; очередь можно переставлять drag & drop; ПКМ удаляет запись только из PlayList. Tab = Lyrics ⇄ Karaoke. | 5.1.17
Karaoke Song Finished: окно открывается мгновенно без ожидания /projects; конец песни срабатывает один раз; Cancel не вызывает повторное открытие; кнопки всегда восстанавливаются при следующем показе; Next проверяет очередь только при нажатии. | 5.1.18
Full Screen Stop: завершает воспроизведение, переводит транспорт в конец песни и сразу открывает Song finished; возврат в начало остаётся отдельной кнопкой Back. | 5.1.18
PROJECT SAVE/LOAD: Projects отделён от Full Screen PlayList. По умолчанию C:\MyNus\Projects; путь SAVE можно изменить; повторное имя — Overwrite / Rename / Save Copy / Cancel. LOAD полностью заменяет отсутствующие дорожки. Project SAVE берёт текущее состояние Lyrics. На стартовом экране добавлен Open Project. | 5.1.17
KARAOKE LOAD: progress 0–100% with phases Opening project / Loading Original / Separating stems / Loading tracks / Restoring Sequencer / Drawing waveforms / Loading Lyrics / Making structure / Preparing Karaoke / Using Effects / Ready. | 5.1.17
</div>
